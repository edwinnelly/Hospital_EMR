...
$assets_list_n= $add_user->hospital_list_charges($hos_key);
?>
<!DOCTYPE html>
<html>
<head>
                    ...
                    <div class="mail-box">
                        ...
                        <table class="table table-hover table-mail dataTables-example">
                            ...
                            foreach ($assets_list_n as $key => $values) {
                                ...
                                <tr class="gradeA">
                                    <td><?= $i; ?></td>
                                    <td><?=  $values->charges_name;  ?></td>
                                    <td><?=  number_format($values->charges);  ?></td>
                                    <td><?=  $values->dated_added;  ?></td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="edit_charges.php?aset=<?=  base64_encode($values->id);  ?>" class="dropdown-item font-weight-bold edit_asset" >Edit Asset</a></li>
...
<script>
    $(document).ready(function () {
        ...
        $("#add_charges").click(function () {
            ...
            if(charges_name=== ''){
                swal("Almost there!", "Please enter charges name!", "info");
            }else{
                $.ajax({
                    url: "ajax/add_charges.php",
                    method: "GET",
                    data: {
                        charges_name: charges_name,charges_amount:charges_amount
                    },
                    success: function (data) {
                        swal("Good job!", "Charges Created!", "success");
                        setTimeout(
                            function () {
                                location.reload();
                            }, 3000);
        ...
        $(".del_charges").click(function () {
            ...
            $.ajax({
                url: "ajax/d_charges.php",
                method: "GET",
                data: {
                    host_fib: host_fib,
                },
                success: function (data) {
                    swal("Good job!", "This Charge Has Been Removed", "success");
                    setTimeout(
                        function () {
                            location.reload();
                        }, 3000);
    ...
    <div class="modal-dialog">
        <div class="modal-content animated flipInY">
            <div class="modal-header">
                ...
                <p class="modal-title font-weight-bold">Create Charges</p>
                <small class="font-bold">You can add and assign charges.</small>
            </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-white btn-outline btn-rounded" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary btn-outline font-weight-bold btn-rounded" id="add_charges">Create Charges</button>
            </div>
        </div>
    </div>
</div>
